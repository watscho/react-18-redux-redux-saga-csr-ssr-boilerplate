...
function* fetchAuth({ payload }: any) {
  try {
    ...
    const response: { [key: string]: any } = yield call(authService.fetchUser, {
      cookies
    })
...
function* logoutAuth({ payload }: any) {
  try {
    const cookies: Generator = yield select(({ cookies }) => cookies)

    const response: { [key: string]: any } = yield call(authService.logoutAuth, { cookies })
    ...
    yield put(authLogoutSucceeded(0))
  } catch (errors) {
